...
\begin{lemma}
  Let $\mathfrak{A}=(Q,\Sigma,q_0,\delta,c),c:Q\rightarrow \{0,\dots,k\}$ be a minimal DPA and $(\mathfrak{A}_0,\dots,\mathfrak{A}_k)$ be its DFA representation. Let $l\subseteq Q$ be a set of states such that for all $q\in l$ there exists a word $w\in\Sigma^+$ and $\delta^+(q,w)=q$.
  ...
  \begin{proof}
    ...
    If the nodes $q,q^\prime\in l$ can be merged, then three conditions hold:
    \begin{enumerate}
      \item $|l| \mod |w| = 0$
      \item every $|w|$-th state of $l$ must have the color $i$
      \item for all $w\in \Sigma^\star$ must hold $c(q)=i=c(q^\prime)$ then $c(\delta^\star(q,w))i$ if and only if $c(\delta^\star(q^\prime,w))=i$ 
    \end{enumerate}
    The first two aspects were shown in the lemma above. In case the third does not hold there exists a word $w\in \Sigma^\star$ such that $\delta_i^\star(q,w)\in F_i$ and $\delta_i^\star(q^\prime,w)\not\in F_i$, which is not possible and contradicts with the definition of the DFA representation.
  \end{proof}
\end{lemma}
